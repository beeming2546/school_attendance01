...
// POST: นักเรียนส่ง token มาเช็กชื่อ
router.post('/api/scan', requireRole('student'), async (req, res) => {
  const { token } = req.body;
  const studentId = req.session.user.studentid;
  ...
  try {
    ...
    const classroomId = tokenResult.rows[0].classroomid;

    // ❗ ตรวจว่านักเรียนคนนี้อยู่ใน classroom นี้หรือไม่
    ...
    if (studentCheck.rows.length === 0) {
      return res.status(403).json({ error: 'คุณไม่ได้อยู่ในห้องเรียนนี้' });
    }

    // ❗ ตรวจว่านักเรียนคนนี้เคยใช้ token นี้ไปแล้วหรือยัง
    const duplicateCheck = await pool.query(
      `SELECT * FROM AttendanceLogtoken
       WHERE token = $1 AND studentid = $2`,
      [token, studentId]
    );

    if (duplicateCheck.rows.length > 0) {
      return res.status(400).json({ error: 'คุณใช้ token นี้ไปแล้ว' });
    }

    // บันทึกการเช็กชื่อ
    await pool.query(
      'INSERT INTO AttendanceLogtoken (studentid, token) VALUES ($1, $2)',
      [studentId, token]
    );

    // ❗ ถ้าอยากให้ใช้ token ได้แค่ครั้งเดียวต่อ class → อัปเดต is_used เป็น true
    await pool.query(
    ...
    console.error('Error scanning QR token:', err);
    res.status(500).json({ error: 'เกิดข้อผิดพลาดในการเช็กชื่อ' });
  }
});
